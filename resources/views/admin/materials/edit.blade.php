    ...
    <div class="max-w-[1120px] mx-auto px-6 py-12">
        <div class="mb-10">
            <a href="{{ route('admin.materials.index') }}"
                ...
                Back to Materials</a>
            <h1 class="text-[26px] font-[650] tracking-[-0.03em] text-black">Edit Material: {{ $material->name }}</h1>
            <p class="text-[13px] text-black/45 mt-1">Modify properties for an existing 3D printing material.</p>
        </div>
        ...
        <div class="glass-card p-10 sm:p-12 max-w-2xl">
            <form action="{{ route('admin.materials.update', $material) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="type" name="type" value="{{ old('type', $material->type) }}"
                        placeholder="e.g., Engineering" class="input-glass h-[52px] px-5 focus:ring-0">
                    @error('type') <p class="text-red-500 text-[11px] mt-1 pl-1">{{ $message }}</p> @enderror
                </div>

                <!-- Colors Configuration -->
                <div x-data="{
                    colors: {{ $material->colors->map(fn($c) => ['id' => $c->id, 'name' => $c->name, 'hex' => $c->hex_code])->toJson() }},
                    addColor() {
                        this.colors.push({ id: null, name: '', hex: '#888888' });
                    },
                    removeColor(index) {
                        this.colors.splice(index, 1);
                    }
                }" x-init="if(colors.length === 0) addColor()">
                        ...
                        <template x-for="(color, index) in colors" :key="index">
                            <div class="flex gap-3 items-center">
                                ...
                                <div class="relative w-[52px] h-[52px] shrink-0 group">
                                    <!-- Color Input (Only active if not transparent) -->
